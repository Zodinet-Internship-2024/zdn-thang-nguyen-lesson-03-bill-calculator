document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateMoneyValues() {
    ...
    errorSpanBill.textContent = ""; // Clear error message initially
    errorSpanPerson.textContent = "";

    if (isNaN(billValue) || billValue === 0) {
      errorSpanBill.textContent = "Can't be zero"; // Set error message
      billInput.style.border = "1px solid red"; // Add red border
    } else {
      billInput.style.border = ""; // Remove red border (if present)
    }
    if (isNaN(numberOfPeople) || numberOfPeople === 0) {
      errorSpanPerson.textContent = "Can't be zero"; // Set error message
      personInput.style.border = "1px solid red"; // Add red border
    } else {
      personInput.style.border = ""; // Remove red border (if present)
    }
